...
type Orchestrator struct {
	...
	// Caveman mode: bypass LLM planning and dispatch all specialists directly
	cavemanMode bool
}
...
// SetCavemanMode enables caveman mode: all specialist agents are dispatched directly
// against the target without any LLM-driven reconnaissance or planning phases.
func (o *Orchestrator) SetCavemanMode(enabled bool) {
	o.cavemanMode = enabled
}

// buildCavemanAgentSpecs returns a comprehensive set of specs covering all major
// vulnerability classes for direct dispatch in caveman mode (no LLM planning).
func buildCavemanAgentSpecs(target string) []SwarmAgentSpec {
	entries := []struct {
		typ      string
		priority string
		context  string
	}{
		{"XSS", "high", "Test for reflected, stored, and DOM-based XSS across all parameters and input vectors."},
		{"SQLi", "critical", "Test for SQL injection using error-based, boolean-blind, time-based, and union-based techniques."},
		{"SSRF", "high", "Test for server-side request forgery via URL parameters, headers, and redirect chains."},
		{"LFI", "high", "Test for local file inclusion and path traversal vulnerabilities across file parameters."},
		{"RCE", "critical", "Test for remote code execution via command injection in all user-controlled input."},
		{"XXE", "high", "Test for XML external entity injection in XML-accepting endpoints and content-type parsers."},
		{"Open Redirect", "medium", "Test for open redirect vulnerabilities via URL and redirect parameters."},
		{"IDOR", "high", "Test for insecure direct object references across all object identifiers."},
		{"CSTI", "medium", "Test for client-side and server-side template injection vulnerabilities."},
		{"Header Injection", "medium", "Test for HTTP header injection and CRLF injection in header-reflected inputs."},
		{"Prototype Pollution", "medium", "Test for JavaScript prototype pollution vulnerabilities in JSON and query params."},
		{"JWT", "high", "Analyze JWT tokens for algorithm confusion, weak secrets, and signature bypass."},
		{"File Upload", "high", "Test for unrestricted file upload allowing shell upload or path traversal."},
		{"API Security", "high", "Test for broken authentication, BOLA, and excessive data exposure in API endpoints."},
		{"Mass Assignment", "medium", "Test for mass assignment vulnerabilities in REST API request bodies."},
		{"Nuclei", "high", "Run Nuclei templates for CVE detection, misconfigurations, and default credentials."},
		{"GoSpider", "medium", "Crawl all accessible endpoints and enumerate hidden paths."},
		{"Asset Discovery", "medium", "Discover subdomains, virtual hosts, and exposed assets."},
		{"Auth Discovery", "high", "Identify authentication endpoints and test for credential weaknesses."},
		{"Resource Hunter", "medium", "Hunt for exposed sensitive files, backups, and developer artifacts."},
		{"Business Logic", "high", "Test for business logic flaws and workflow bypass vulnerabilities."},
		{"DASTySAST", "medium", "Run dynamic analysis to surface common vulnerabilities from application behavior."},
	}

	specs := make([]SwarmAgentSpec, 0, len(entries))
	for _, e := range entries {
		specs = append(specs, SwarmAgentSpec{
			Type:       e.typ,
			Target:     target,
			Context:    e.context,
			Hypothesis: fmt.Sprintf("Caveman dispatch: probe %s vulnerabilities against %s", e.typ, target),
			Priority:   e.priority,
		})
	}
	return specs
...
// runCavemanFlow executes a caveman-mode scan: all specialist agents are dispatched
// directly without LLM reconnaissance or planning. Findings are collected and reported.
func (o *Orchestrator) runCavemanFlow(ctx context.Context, flowID uuid.UUID, userPrompt string) error {
	...
	task, err := o.queries.CreateTask(flowID, "Penetration Test [Caveman Mode]", userPrompt)
	if err != nil {
	...
	o.bus.Subscribe(EventFindingDiscovered, func(data interface{}) {
		...
		o.emit(flowID.String(), Event{
			...
			FlowID:  flowID.String(),
			TaskID:  task.ID.String(),
			Content: fmt.Sprintf("[BRAIN] Synapse [finding]: %s %s", finding.Type, finding.URL),
		})
	})
	...
	if err := o.pipeline.Start(); err != nil {
		log.Printf("[caveman] pipeline start: %v", err)
	}
	...
	o.emit(flowID.String(), Event{
		...
		FlowID:  flowID.String(),
		TaskID:  task.ID.String(),
		Content: "[CAVEMAN] Caveman Mode activated — bypassing LLM planning, dispatching all specialists directly.",
	})

	// Skip recon and strategy; advance pipeline directly to exploitation
	for _, reason := range []string{"caveman: skip recon", "caveman: skip discovery", "caveman: skip strategy"} {
		if err := o.pipeline.Advance(reason, nil); err != nil {
			log.Printf("[caveman] pipeline advance (%s): %v", reason, err)
		}
	}
	o.emitPipelineEvent(flowID.String(), task.ID.String())

	agentSpecs := buildCavemanAgentSpecs(flow.Target)
	agentSpecs = filterDispatchSpecs(agentSpecs, make(map[string]struct{}), flow.Target, base.BrowserAvailable())

	o.emit(flowID.String(), Event{
		Type:     EventMessage,
		FlowID:   flowID.String(),
		TaskID:   task.ID.String(),
		Content:  fmt.Sprintf("[CAVEMAN] Dispatching %d specialist agents against %s", len(agentSpecs), flow.Target),
	...
	for _, spec := range agentSpecs {
		...
		specialistSubTask, err := o.createLedgerSubTask(
			task.ID,
			nil,
			fmt.Sprintf("%s Specialist [Caveman]", spec.Type),
			spec.Context,
			...
			dispatchKey,
			spec.Context,
			map[string]any{"mode": "caveman", "spec_type": spec.Type},
		)
		if err != nil {
			log.Printf("[caveman] failed to create subtask for %s: %v", spec.Type, err)
			continue
		}

		payload := buildWorkerPayload(flow.Target, spec, spec.Context, nil)
		...
		if err := o.queueMgr.Route(queueName, payload, flowID.String()); err != nil {
			log.Printf("[caveman] route to %s failed: %v", queueName, err)
			continue
		}
	}
	...
	// Reporting phase
	for _, reason := range []string{"caveman: exploitation complete", "caveman: validation", "caveman: reporting"} {
		if err := o.pipeline.Advance(reason, nil); err != nil {
			log.Printf("[caveman] pipeline advance (%s): %v", reason, err)
		}
	}
	...
	if dedupedCount < originalCount {
		o.emit(flowID.String(), Event{
			...
			FlowID:  flowID.String(),
			TaskID:  task.ID.String(),
			Content: fmt.Sprintf("[DEDUP] Compressed %d findings into %d unique root causes.", originalCount, dedupedCount),
		})
	}
	...
	finalReport := o.reporter.GenerateReport(
		...
		brain.Exclusions,
		swarmRes,
		"[Caveman Mode] Direct specialist dispatch — no LLM-generated PoC.",
	)
	...
	if err := o.pipeline.Advance("caveman: report generated", map[string]interface{}{
		"total_findings": len(brain.Findings),
		"total_leads":    len(brain.Leads),
		"duration_sec":   o.pipeline.TotalDuration().Seconds(),
	}); err != nil {
		log.Printf("[caveman] pipeline advance (complete): %v", err)
	}
	o.emitPipelineEvent(flowID.String(), task.ID.String())
	...
	o.emit(flowID.String(), Event{
		...
		FlowID:  flowID.String(),
		TaskID:  task.ID.String(),
		Content: "Caveman Mode Assessment Complete",
	})
...
func (o *Orchestrator) RunFlow(ctx context.Context, flowID uuid.UUID, userPrompt string) error {
	if o.cavemanMode {
		return o.runCavemanFlow(ctx, flowID, userPrompt)
	}
