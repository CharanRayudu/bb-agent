...
def persist_dir(root: str) -> str:
    return os.path.join(root, "rag_service", "faiss_db")


def manifest_path(root: str) -> str:
    return os.path.join(root, "rag_service", "faiss_db", "faiss_manifest.json")


def index_path(root: str) -> str:
    return os.path.join(root, "rag_service", "faiss_db", "kb.faiss")


def store_path(root: str) -> str:
    return os.path.join(root, "rag_service", "faiss_db", "kb_store.json")
...
def create_embedder(model_dir: str = None) -> object:
    ...
    try:
        ...
        # 确保 rag 模块在路径中
        rag_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "rag_service")
...
def main():
    ...
    # 优先使用本地模型目录
    local_model_dir = os.path.join(root, "rag_service", "models", "all-MiniLM-L6-v2")
    ...
    print(f"[INFO] 当前嵌入模型维度: {embed_dim}")
    ...
    if os.path.isfile(index_fp):
        try:
            temp_index = faiss.read_index(index_fp)
            if hasattr(temp_index, 'd') and temp_index.d != embed_dim:
                print(f"[WARN] 现有FAISS索引维度 ({temp_index.d}) 与当前模型维度 ({embed_dim}) 不匹配！(比如从本地模型切换到了NVIDIA模型)")
                print(f"[WARN] 将强制重建整个知识库...")
                force_all = True
            else:
                index = temp_index
                print(f"[INFO] 加载现有索引，现有向量数: {index.ntotal}")
        except Exception as e:
            print(f"[WARN] 读取现有索引出错: {e}，将创建新索引。")

    if index is None:
        base_index = faiss.IndexFlatIP(embed_dim)
        index = faiss.IndexIDMap2(base_index)
        print(f"[INFO] 创建新的空索引，维度: {embed_dim}")
