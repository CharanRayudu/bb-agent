...
try:
    from rag_service.model_manager import get_embedding_model, get_model_dim
except Exception:
...
class HybridQueryEmbedder:
    ...
    def encode(self, texts: List[str]) -> List[List[float]]:
        if self.model is not None:
            try:
                # The new BaseEmbedder interface handles conversion internally
...
class RAGClient:
    ...
    def build_index(self) -> None:
        ...
        if not self.is_available():
            return
        db_path = os.path.join(self.base_dir, "rag_service", "faiss_db")
