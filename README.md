...
Mirage is a black-box penetration testing platform powered by large language models. It autonomously plans, executes, and validates vulnerability scans against live web targets using a multi-phase pipeline, 32 specialist agents, headless browser validation, and a real-time React dashboard.
...
React + Vite Frontend
  ...
  -> Go backend (REST + WebSocket API)
     -> orchestrator and 8-phase pipeline
     -> 32 specialist agents (XSS, SQLi, SSRF, ...)
     -> schema validation and per-agent queueing
